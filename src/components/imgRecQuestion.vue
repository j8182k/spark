...
const submitForm = async () => {
    if (selectedFile.value) {
        ...
        try {
            ...
            const response = await request.post('http://localhost:8080/uploadImg', params, {
                headers: {
...
const Qconfirm = async()=>{
    if(dic.value.Q!==''){
        const params = new FormData();
        const s = "{ \"Q\":\""+dic.value.Q+"\",\"A\":\""+dic.value.A+"\",\"B\":\""+dic.value.B+"\",\"C\":\""+dic.value.C+"\",\"D\":\""+dic.value.D+"\",\"answer\":\""+dic.value.answer+"\",\"course\":\""+dic.value.course+"\",\"mu\":\""+dic.value.mu+"\",\"sigma\":\""+dic.value.sigma+"\"}"
        params.append('question', s)

        try {

            isLoading.value = true
            const response = await request.post('http://localhost:8080/upLoadQuestion', params, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            })
...
<template>
  ...
  <div class="center"><h1>上传题目</h1></div>
        ...
        >
          ...
          <el-form-item label="题目难度识别">
              <div class="slider-demo-block">
                  ...
                  <span class="demonstration">{{ (dic.mu >= 20 && dic.mu < 30 )? '正常':dic.mu >= 0 && dic.mu <20 ? '简单':dic.mu >= 30 && dic.mu < 50 ? '较难': dic.mu >= 50 && dic.mu < 70 ? '困难':'难出天际'}}</span>
              </div>
          </el-form-item>
          <el-form-item label="题目难度分析:">
            {{ dic.message }}
          </el-form-item>
      ...
      <div style="display: flex;flex-direction: column;margin-left: 5%;">
        <div style="display: flex;">
        <el-upload class="upload-demo" drag action="#" multiple :limit="1" :file-list="fileList"
              :http-request="handleFileUpload" :on-exceed="handleExceed">
              <el-icon class="el-icon--upload"><upload-filled /></el-icon>
              <div class="el-upload__text">
                  把图片拖拽到此处或 <em>点击上传</em>
              </div>
              <template #tip>
